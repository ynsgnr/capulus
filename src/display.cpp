...
void CAPULUS_DISPLAY::state(stateData data, float currentTemp)
{
    ...
    case SELECT_PRESS:
        ...
        if (data.selected == SELECT_TEMP)
            capulus_display.print(F(SELECTOR));
    ...
    case SELECT_PREINF_TIMER:
        page = 1;
        if (data.selected == SELECT_PREINF_PRESS)
            capulus_display.print(F(SELECTOR));
        ...
        capulus_display.println(F(BAR_TEXT));
        if (data.selected == SELECT_PREINF_TIMER)
            capulus_display.print(F(SELECTOR));
        capulus_display.setTextSize(TEXT_SIZE / 2, TEXT_SIZE);
    ...
    case SELECT_STEAM_TEMP:
        page = 2;
        if (data.selected == SELECT_BREW_TIMER)
            capulus_display.print(F(SELECTOR));
        capulus_display.setTextSize(TEXT_SIZE / 2, TEXT_SIZE);
        ...
        capulus_display.println(F(SECONDS_TEXT));
        if (data.selected == SELECT_STEAM_TEMP)
            capulus_display.print(F(SELECTOR));
        capulus_display.setTextSize(TEXT_SIZE / 2, TEXT_SIZE);
    ...
    case SELECT_SLEEP_TIMER:
        page = 3;
        if (data.selected == SELECT_SLEEP_TIMER)
            capulus_display.print(F(SELECTOR));
        capulus_display.setTextSize(TEXT_SIZE / 2, TEXT_SIZE);
