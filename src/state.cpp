...
void CAPULUS_STATE::input(inputData input)
{
    ...
    if (sdata.temp > MAX_TEMP)
        sdata.temp = MAX_TEMP;
    if (sdata.pressure > MAX_PRESS)
        sdata.pressure = MAX_PRESS;
    if (sdata.preinfusionPressure > MAX_PRESS)
        sdata.preinfusionPressure = MAX_PRESS;
    if (sdata.preinfusionTimerSeconds > MAX_BREW_TIME)
        sdata.preinfusionTimerSeconds = MAX_BREW_TIME;
    if (sdata.brewTimerSeconds > MAX_BREW_TIME)
        sdata.brewTimerSeconds = MAX_BREW_TIME;
    if (sdata.steamTemp > MAX_TEMP)
        sdata.steamTemp = MAX_TEMP;
    if (sdata.sleepTimerMinutes > MAX_SLEEP_TIME)
        sdata.pressure = MAX_SLEEP_TIME;
}
